...
export const AppProvider = ({ children }) => {
  ...
  const initializeContext = useCallback(async () => {
    try {
      ...
      const userRole = Number(me?.role ?? -1);
      
      setCurrentUserId(resolvedId);
      currentUserIdRef.current = resolvedId;

      if (resolvedId && userRole !== 0) { // Only load courses for regular users (not admin)
        ...
        // For admin users, don't try to load courses
        setMyCourses([]);
        coursesLoadedRef.current = false;
      }
  ...
  // Listen for route changes to re-initialize when switching between admin/user
  useEffect(() => {
    const handleRouteChange = () => {
      // Re-initialize context when navigating between different areas
      const currentPath = window.location.pathname;
      if (currentPath.includes('/admin') && myCourses.length > 0) {
        // Don't clear courses when going to admin, just pause updates
        return;
      } else if (!currentPath.includes('/admin') && currentUserId) {
        // Ensure courses are loaded when coming back to user area
  ...
  }, [currentUserId, myCourses.length, refreshEnrollments]);
