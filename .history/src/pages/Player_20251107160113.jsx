...
const Player = () => {
  ...
  const renderVideoPlayer = () => {
    ...
    const videoInfo = selectedLesson.videoInfo;
    console.log('Rendering video with info:', videoInfo); // Debug video rendering

    // Handle embedded videos (YouTube, Vimeo, etc.)
    if (['youtube', 'vimeo', 'googledrive', 'dailymotion'].includes(videoInfo.type)) {
      return (
        <iframe
    ...
    // Handle direct video files (both uploaded files and direct URLs)
      ...
      return (
        <div className="w-full h-full relative">
          <video
            ...
            src={videoInfo.src}
            onLoadStart={() => console.log('Video started loading:', videoInfo.src)}
            onCanPlay={() => console.log('Video can play:', videoInfo.src)}
            onError={(e) => {
              console.error('Video loading error:', e);
              console.error('Video src:', videoInfo.src);
              console.error('Video info:', videoInfo);
            }}
          >
            <source src={videoInfo.src} type={`video/${videoInfo.extension || 'mp4'}`} />
            متصفحك لا يدعم تشغيل الفيديو
          </video>
          {/* Error fallback */}
          <div className="absolute inset-0 hidden items-center justify-center text-white bg-black bg-opacity-75" id="video-error">
            <div className="text-center">
              <p className="text-sm">خطأ في تحميل الفيديو</p>
              <p className="text-xs mt-1 opacity-75">تحقق من اتصال الإنترنت</p>
            </div>
          </div>
        </div>
      );
    ...
    return (
      <div className="w-full h-full flex items-center justify-center text-white">
        <div className="text-center">
          <p className="text-sm opacity-75">تنسيق الفيديو غير مدعوم</p>
          <p className="text-xs mt-2 opacity-50">نوع الفيديو: {videoInfo.type}</p>
        </div>
      </div>
    );
  };
