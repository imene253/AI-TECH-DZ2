...
const MyCourses = () => {
  ...
  const testCoursesDirectly = async () => {
    try {
      ...
      setDebugInfo(`User ID: ${userId}, Role: ${userRole} (${userRole === 0 ? 'Admin' : 'User'})`);

      // Check if admin is trying to access user courses
      if (userRole === 0) {
        setDebugInfo(prev => prev + `\n⚠️ You are logged in as ADMIN (User ID: ${userId})`);
        setDebugInfo(prev => prev + `\nAdmins don't have course enrollments.`);
        setDebugInfo(prev => prev + `\nTo see courses, sign in as a regular user.`);
        return;
      }

      // Test enrollments for regular users
      try {
      ...
      <section className="max-w-[1120px] w-full mx-auto px-6 sm:px-8 md:px-10 lg:px-16 pt-10 pb-6">
        ...
        <div className="bg-yellow-100 p-3 rounded-lg mb-4 text-right">
          <p className="text-sm">
            ...
            User Type: {isAdminUser ? 'Admin' : 'Regular User'}<br/>
