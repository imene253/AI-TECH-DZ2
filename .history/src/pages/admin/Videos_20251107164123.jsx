...
const Videos = () => {
  ...
  const onCreate = async (e) => {
    ...
    try {
      ...
      console.log('Chapter ID:', form.chapterId, typeof form.chapterId);
      console.log('Video File:', {
        name: form.videoFile?.name,
        size: form.videoFile?.size,
        type: form.videoFile?.type,
        lastModified: form.videoFile?.lastModified
      });

      
      const formData = new FormData();
      formData.append('Url', 'placeholder-url'); 
      ...
      console.log('Making request to /api/Video...');
      const response = await api.post('/api/Video', formData);
      console.log('Video creation successful:', response);
      ...
      if (e?.data) {
        try {
          const errorDetails = typeof e.data === 'string' ? e.data : JSON.stringify(e.data);
          console.error('Detailed error response:', errorDetails);
          setError(`Create failed: ${e?.message || 'Unknown error'}. Details: ${errorDetails}`);
        } catch (parseErr) {
