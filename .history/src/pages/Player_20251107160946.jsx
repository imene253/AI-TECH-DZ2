...
const Player = () => {
              ...
              try {
                ...
                return {
                  ...
                  lessons: filteredVideos.map(video => {
                    console.log('Processing video:', video); // Debug video data
                    return {
                      id: video.id,
                      title: video.title || `فيديو ${video.id}`,
                      url: video.url,
                      videoFile: video.videoFile || video.VideoFile, // Handle different field name cases
  ...
  const getVideoInfo = (url, videoFile) => {
    console.log('getVideoInfo called with:', { url, videoFile }); // Debug input
    
    // Priority: uploaded video file > URL
    if (videoFile) {
      const videoFileUrl = getVideoUrl(videoFile);
      console.log('Processed video file URL:', videoFileUrl); // Debug processed URL
      if (videoFileUrl) {
  ...
  const renderVideoPlayer = () => {
    ...
    // Debug selected lesson data
    console.log('Selected lesson data:', selectedLesson);
    console.log('Video file path:', selectedLesson?.videoFile);
    console.log('URL:', selectedLesson?.url);

    // Use the same video handling logic as CourseDetail (which works)
    return (
      <div className="w-full h-full">
        {(() => {
          // Try video file first, then fallback to URL
          const videoFile = getVideoUrl(selectedLesson?.videoFile);
          const url = selectedLesson?.url || '';

          console.log('Processed video file URL:', videoFile);
          ...
          if (videoFile && videoFile !== 'http://iatech.runasp.net/placeholder-url') {
            const isVideoFile = /\.(mp4|webm|ogg)$/i.test(videoFile);
            console.log('Video file check:', { videoFile, isVideoFile });
            
            if (isVideoFile) {
              console.log('Attempting to play uploaded video file:', videoFile);
              ...
              return (
                <video 
                  ...
                  onLoadStart={() => console.log('Video load started for:', videoFile)}
                  onLoadedMetadata={() => console.log('Video metadata loaded for:', videoFile)}
                  onCanPlay={() => console.log('Video can play:', videoFile)}
                  onError={(e) => {
          ...
          if (yt) {
            console.log('Playing YouTube video:', yt);
            return (
              <iframe
                className="w-full h-full"
                src={yt}
          ...
          if (isVideoFile && url !== 'placeholder-url') {
            ...
            return (
              <video 
                ...
                controls 
                controlsList="nodownload"
                onError={(e) => console.error('Direct video error:', e)}
              />
            );
          }
          ...
          return (
            <div className="w-full h-full flex items-center justify-center text-white">
              <div className="text-center">
                <p className="text-sm opacity-75">لا يوجد رابط فيديو متاح</p>
                <p className="text-xs mt-2 opacity-50">Debug: videoFile={videoFile}, url={url}</p>
              </div>
            </div>
          );
        })()}
